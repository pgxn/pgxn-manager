-- sql/18-ownership.sql SQL Migration

-- Drop previous versions of the functions.
SET client_min_messages = warning;
DROP FUNCTION IF EXISTS is_admin_or_owns(LABEL, TEXT[]);
DROP FUNCTION IF EXISTS grant_coownership(LABEL, LABEL, TEXT[]);
DROP FUNCTION IF EXISTS revoke_coownership(LABEL, LABEL, TEXT[]);
DROP FUNCTION IF EXISTS transfer_ownership(LABEL, LABEL, TEXT[]);
...
REVOKE ALL ON FUNCTION is_admin_or_owns(LABEL, TERM[]) FROM PUBLIC;
